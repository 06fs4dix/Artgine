const CACHE_NAME = "ServiceWorker-1";
		const MAX_CACHE_SIZE = 52428800;
		const LOG = true;
    ...
    event.waitUntil(
        Promise.all([
            caches.keys().then((cacheNames) => {
                return Promise.all(cacheNames.filter(cache => cache !== CACHE_NAME).map(cache => caches.delete(cache)));
            }),
            InitCacheInfo()
        ])
    );
...
self.addEventListener("message", (event) => 
{
    ...
    {
        event.source.postMessage({type: "INIT_PWA",data:CheckServerStatus()});
    } 
...
async function AddCache(request, response, cache) 
{
    ...
    try {
        ...
        SWLog(`캐시 추가: ${url} (${DataSizeFormat(size)}), 총 ${DataSizeFormat(gCacheUsageInfo.totalSize)}`);
