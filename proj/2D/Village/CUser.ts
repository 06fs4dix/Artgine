...
export class CUser extends CSubject
{
    ...
    ResetAnimation(_key: string)
    {
        const isMove = _key.startsWith("Move");
        this.mState  = isMove ? "walk" : "idle";

        if      (_key.endsWith("Down"))  this.mDir = CVec3.eDir.Down;
        else if (_key.endsWith("Up"))    this.mDir = CVec3.eDir.Up;
        else if (_key.endsWith("Left"))  this.mDir = CVec3.eDir.Left;
        else if (_key.endsWith("Right")) this.mDir = CVec3.eDir.Right;
    }
