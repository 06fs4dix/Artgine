...
// 					if(this.mNormal)
// 						L_nor = this.mNormal;
// 					// --- [추가/수정된 부분] mNorList가 있으면 카메라 크로스 연산을 무시하고 해당 노말을 사용 ---
// 					else if(this.mNorList && this.mNorList.length > 0)
// 					{
// 						L_nor = CUtilMath.WeightVec3(this.mNorList, i / this.mVCount);
// 					}
// 					// ------------------------------------------------------------------------------------------
// 					else if(this.mPosList.length>this.mVCount)
...
export class CPaintTrail extends CPaint
{
	public mOut : CFloat32Mgr=new CFloat32Mgr();
	public mPosListCount=new CVec1();
	public mCurrentTargetSize: number = 0;
	public mIsOutChanged: boolean = false;
	public mLastHide=false;
	...
	UpdateBuffer(_upPos: Array<CVec3>, _downPos: Array<CVec3>) 
	{
		this.mBW.mRadian = 0;
		this.mBound.Reset();
		this.mBound.SetType(CBound.eType.Box);
		...
		// 1. 현재 입력된 포지션 개수에 필요한 타겟 사이즈 계산 (4의 제곱수)
		let neededSize = 4;
		while (neededSize < _upPos.length) {
			neededSize *= 4;
		}

		// 2. 버퍼 확장 로직: 필요한 사이즈가 현재 버퍼 크기보다 클 때만 확장 (축소 시에는 유지)
		if (this.mCurrentTargetSize < neededSize) {
			this.mCurrentTargetSize = neededSize;
			
			// CFloat32Mgr 사이즈 변경 (*4는 V4 데이터, *2는 Up/Down 두 줄을 의미)
		...
		// 3. mOut 버퍼에 데이터 채우기 
		// 루프는 현재 할당된 최대 크기인 mCurrentTargetSize만큼 돌아서 나머지 영역을 정리함
		for (let i = 0; i < this.mCurrentTargetSize; i++) 
		{
			...
			{
				// 실제 데이터 범위를 벗어난 영역은 마지막 정점 데이터로 채워 렌더링 오류 방지
	...
	override Render(_vf : CShader)
	{
		if (this.mCurrentTargetSize === 0) return;
		...
		this.mOwner.GetFrame().BMgr().SetBatchSA(new CShaderAttr("texCodi",this.mTexCodi));
		//this.mOwner.GetFrame().BMgr().SetBatchSA(new CShaderAttr("trailWidth",100));
		//this.mOwner.GetFrame().BMgr().SetBatchSA(new CShaderAttr("posListCount",this.mPosListCount));
		this.mOwner.GetFrame().BMgr().SetBatchSA(new CShaderAttr("trailPos",4,this.mOut.GetArray()));
		if(this.mLastHide)
            this.mOwner.GetFrame().BMgr().SetBatchSA(new CShaderAttr("lastHide",new CVec1(1.0)));
		else
