...
import { SDF } from "./SDF";
import { Attribute, BranchBegin, BranchEnd, Build, CVec2, CVec3, CVec4, FloatToInt, IntToFloat, MappingTexToV3, Null, Sam2D0ToColor, Sam2DSize, Sam2DToColor, SaturateV4, V2AddV2, V2DivV2, V2MulFloat, V2MulV2, V3AddV3, V3Dot, V3MulFloat, V4AddV4, V4DivV4, V4MulFloat, V4MulMatCoordi, clamp, discard, max, pow } from "./Shader";
var worldMat = Null();
var viewMat = Null();
var projectMat = Null();
...
Build("Artgine/Shader/PostBRDF", ["brdf"], vs_main, [
    worldMat, viewMat, projectMat
], [out_position, to_uv], ps_main_brdf, [out_color]);
...
function ps_main_light() {
    ...
    var dseMat = LightCac3D(camPos3D, worldPos, L_dif, Normal, shadow, L_spc.y, L_spc.x, L_spc.z, ambientColor, 1.0);
    if (renType < 0.5)
        L_cor.rgb = dseMat[0];
    else if (renType < 1.5) {
        L_cor.rgb = dseMat[1];
...
function ps_main_light_MultiTex() {
    ...
    var dseMat = LightCac3D(camPos3D, worldPos, L_dif, Normal, shadow, SpecularStrength, Emissive, SpecularPower, ambientColor, 1.0);
...
function ps_main_brdf() {
    var integratedBRDF = IntegrateBRDF(to_uv.x, to_uv.y);
