...
function ProcessCascadeLevel(_isActive, _viewMatOff, _projMatOff, _world, _shadowIndex) {
    if (_isActive < -0.5)
        return new CVec3(0.0, 0.0, 0.0);
    var viewPos = V4MulMatCoordi(_world, Sam2DArrToMat(_viewMatOff, _shadowIndex));
...
function CalcShadowDirectional(_read, _index, _world, _ligDir, _bias) {
    var uvZ0 = ProcessCascadeLevel(_read.y, shadowNearCasV0, shadowFarCasP0, _world, _index);
    var uvZ1 = ProcessCascadeLevel(_read.z, shadowTopCasV1, shadowBottomCasP1, _world, _index);
    var uvZ2 = ProcessCascadeLevel(_read.w, shadowLeftCasV2, shadowRightCasP2, _world, _index);
...
function CalcShadowPoint(_read, _index, _world, _ligDir, _bias) {
...
export function CalcShadow(_shadowIndex, _normal, _world) {
    ...
    var normalOffset = V3MulFloat(V3Nor(_normal), normalScale);
    _world.xyz = V3AddV3(_world.xyz, normalOffset);
    var biasConst = bias;
    var biasSlope = bias * clamp(tanTheta * 0.5, 0.0, 2.0);
    var biasAll = new CVec2(biasConst, biasSlope);
    var sVal;
    if (isPointLight > 0.5)
        sVal = CalcShadowPoint(shadowRead, _shadowIndex, _world, lDir, biasAll);
    else
        sVal = CalcShadowDirectional(shadowRead, _shadowIndex, _world, lDir, biasAll);
