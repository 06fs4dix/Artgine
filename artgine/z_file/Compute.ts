...
//워크그룹 크기를 바꾸려면 var wgSize : number = 128; 을 선언한다(기본 64).
//넘치는 스레드를 거르는 경계 검사는 진입점에 자동으로 들어간다
Build("Artgine/Compute/PosTransform",[],
	pos_main,[local,oidx,mats,dst],[],
	null,[]
);

function pos_main()
{
	var b : number=invocationID*3.0;
	//인덱스는 인터프리터가 정수로 바꿔주므로 여기서 따로 변환하지 않는다
	var o : CVec4=V4MulMatCoordi(new CVec4(local[b],local[b+1.0],local[b+2.0],1.0),
		mats[oidx[invocationID]]);
	dst[b]=o.x;
	dst[b+1.0]=o.y;
	dst[b+2.0]=o.z;
}
