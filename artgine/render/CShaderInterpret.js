...
export class CShaderInterpret {
    ...
    async ExeOne(_fileName, _source) {
        return null;
    }
    async ExeAll(_fileName, _source, _load = null) {
        return null;
    }
    async BuildAST(_source) {
        return null;
    }
    BuildIR(_ast) {
        return null;
    ...
    VFPasing(_str, _vfCount, vf = new CVertexFormat()) {
        return null;
    }
...
export class CShaderInterpretGL extends CShaderInterpret {
    ...
    AttachFun(_useFun, _functionMap, _addedFun = null) {
        if (_addedFun === null) {
            _addedFun = [];
        }
        let funStr = "";
...
export class CShaderInterpretGPU extends CShaderInterpret {
    ...
    mTexArrCount = 0;
    mTexCubeCount = 0;
    static TexBinding(_kind, _index, _texMax, _arrMax) {
        if (_kind == "2d")
            return _index * 2;
        if (_kind == "arr")
            return _texMax * 2 + _index * 2;
    ...
    BuildVSUni(_shader, _in, _compute = false) {
        return "";
    }
    Build() {
    }
