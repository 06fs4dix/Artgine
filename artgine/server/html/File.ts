...
function promptFileAuth(onSuccess?: () => void) {
    ...
    const doAuth = () => {
        ...
        authLogin(g_fileWebRootUrl, CHash.SHA256('artgine_' + pw),
            () => { CAlert.Info("Waiting for messenger approval (up to 5 minutes)..."); }
        ).then(async (j: { ok: boolean, token?: string, msg?: string }) => {
            if (j.ok) {
