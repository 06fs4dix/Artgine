let MODELS = { claude: [], codex: [], antigravity: [], opencode: [], grok: [], cmd: [] };
...
function renderMarkdown(s) {
    return marked.parse(escapeHtml(s), { xhtml: false });
}
...
async function fetchProviders() {
    try {
        ...
        for (const id of ['claude', 'codex', 'antigravity', 'opencode', 'grok']) {
            MODELS[id] = models[id] || [];
        }
        return true;
    }
...
function rebuildProviderOptions() {
    ...
    const _providerLabels = {
        claude: 'Claude', codex: 'Codex', antigravity: 'Antigravity', opencode: 'OpenCode', grok: 'Grok', cmd: 'CMD (shell)',
    };
    for (const id of ['claude', 'codex', 'antigravity', 'opencode', 'grok', 'cmd']) {
...
async function bootChat() {
    ...
    if (paramWrite !== null && paramWrite !== undefined)
        sessionWrite = paramWrite !== '0';
