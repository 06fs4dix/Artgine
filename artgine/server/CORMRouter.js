...
let CORMRouter = class CORMRouter extends CAuthServer {
    ...
    async CreateORM(_dbType) {
        switch (_dbType) {
            case 'mysql': {
                const { CMysql } = await import('../network/CMysql.js');
                return new CMysql();
            }
            case 'mssql': {
                const { CMssql } = await import('../network/CMssql.js');
                return new CMssql();
            }
            case 'sqlite': {
                const { CSQLite } = await import('../network/CSQLite.js');
                return new CSQLite();
            }
            case 'ne': {
                const { CNe } = await import('../network/CNe.js');
                return new CNe();
            }
        }
        return null;
    ...
    async onExec(_json, _req, _res) {
        ...
        try {
            ...
            const condition = conditionRaw.map((c) => new CORMCondition(c.mKey, c.mCondition, c.mValue, c.mLogical));
            const data = dataRaw.map((d) => new CORMField(d.mKey, d.mValue));
