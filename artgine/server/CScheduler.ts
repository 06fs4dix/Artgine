...
export class CScheduler {
    ...
    private static async Init(): Promise<CRDBMS> {
        if (CScheduler.sDB != null) return CScheduler.sDB;

        const db = new CSQLite();
        await db.Init();

        await db.CreateCollection(CScheduler.sTable, [
            new CORMField('name', ''),
    ...
    public static async List(): Promise<SchedulerRecord[]> {
        const db = await CScheduler.Init();
        const rows = await db.Recv(`SELECT name, subAgentKey, mode, option, command FROM ${CScheduler.sTable} ORDER BY name ASC`);
        if (rows == null) return [];
        return rows.map(CScheduler.RowToRecord);
    }
    ...
    public static async Set(_record: SchedulerRecord): Promise<void> {
        const db = await CScheduler.Init();
        await db.Send(
            `INSERT INTO ${CScheduler.sTable} (name, subAgentKey, mode, option, command) VALUES (?, ?, ?, ?, ?)
    ...
    public static async Delete(_name: string): Promise<boolean> {
        const db = await CScheduler.Init();
        const rows = await db.Recv(`SELECT name FROM ${CScheduler.sTable} WHERE name = ?`, [_name]);
        if (rows == null || rows.length === 0) return false;
        await db.Send(`DELETE FROM ${CScheduler.sTable} WHERE name = ?`, [_name]);
        return true;
    }
    ...
    private static RowToRecord(_row: any[]): SchedulerRecord {
        return {
            ...
            mode: String(_row[2]),
            option: CScheduler.ParseOption(_row[3]),
