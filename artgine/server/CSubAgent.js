...
export class CSubAgent {
    ...
    static async Set(_record) {
        const db = await CSubAgent.Init();
        await db.Send(`INSERT INTO ${CSubAgent.sTable} (key, provider, model, score, traits, workingDir, \`super\`, retryText, retryCount, permissions, hidden) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
             ON CONFLICT(key) DO UPDATE SET provider = excluded.provider, model = excluded.model, score = excluded.score, traits = excluded.traits, workingDir = excluded.workingDir, \`super\` = excluded.\`super\`, retryText = excluded.retryText, retryCount = excluded.retryCount, permissions = excluded.permissions, hidden = excluded.hidden`, [_record.key, _record.provider, _record.model, _record.score, JSON.stringify(_record.traits), _record.workingDir || './', _record.super ? 1 : 0, _record.retryText || '', _record.retryCount || 0, CSubAgent.StringifyPermissions(_record.permissions), _record.hidden ? 1 : 0]);
    }
