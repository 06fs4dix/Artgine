...
import { CClip, CClipBase64, CClipCoodi, CClipImg, CClipMesh } from "../app/component/CAnimation.js";
...
export function AniToolAtlasEvent(_atl, _push, _event) {
    gAtlas = _atl;
    gAtlasPush = _push;
            ...
            for (let i = 0; i < gAni.mClip.length; ++i) {
                ...
                if (clip instanceof CClipCoodi) {
                    if (clip.mSTX < world.x && world.x < clip.mEDX && clip.mSTY < -world.y && -world.y < clip.mEDY) {
                        const listTab = CDOM.ID("list-tab");
                        if (listTab)
                            listTab.click();
                        setTimeout(() => {
...
function AniToolRangeInit() {
    ...
    let pt = ClipSub.PushComp(new CPaintHTML(CDOM.DataToDom(`<div class='border border-primary' style='color:red;font-size: x-small;pointer-events:none;user-select:none;'></div>`), new CVec2(EDX_num - STX_num, EDY_num - STY_num), CDOM.ID("AniToolLeft_div")));
...
async function AniToolSubjectInit() {
    ...
    for (let i = 0; i < gAni.mClip.length; ++i) {
        ...
        if (clip instanceof CClipImg) {
            ...
            let ClipSub = gAtl.Canvas("AniTool").PushSub(new CSubject());
            ClipSub.SetKey(i);
            let pt = ClipSub.PushComp(new CPaintHTML(CDOM.DataToDom(`<div class='border border-primary' style='color:red;font-size:x-small;overflow:hidden;line-height:1;pointer-events:none;user-select:none;'><span style='display:inline-block;text-shadow:0 0 2px #000,0 0 2px #000;'>${i}</span></div>`), new CVec2(clip.mEDX - clip.mSTX + 2, clip.mEDY - clip.mSTY + 2), CDOM.ID("AniToolLeft_div")));
