...
export function AniToolTexcodiEvent(_texcodi : CPaint2D,_event)
{
...
var gAtl: CAtelier;
var gAni: CAnimation;
var gSubject: CSubject;
//var gTarget: CSubject;
var gMode = 0;
var gImg: CTexture = null;
var gUpdateEvent: CEvent;
var gSelect: CSubject = null;
var gBasicTex=null;
var gBasicMesh=null;
//1레인지,2이동,3생성
export function AniTool(_ani: CAnimation, _basicTex=null,_basicMesh=null) {
    gAni = _ani;
...
<div class="tab-pane fade" id="tab3d" role="tabpanel">
  <div class="card mt-2">
    ...
    gAtl.Init([], "AniToolLeft_can", false).then(()=>{
        ...
        if(gBasicTex==null) gBasicTex=gAtl.Frame().Pal().GetNoneTex();
        if(gBasicMesh==null) gBasicMesh=gAtl.Frame().Pal().GetBoxMesh();
    ...
    const selectEl = CUtil.ID("clipTypeSelect") as HTMLSelectElement;
    const addBtn = CUtil.ID("addClipBtn") as HTMLButtonElement;

    const classMap = CClass.ExtendsList(CClip);  // { CClipColor: class, CClipCoodi: class, ... }
    ...
    addBtn.addEventListener("click", () => {
        ...
        let newClip: CClip = CClass.New(selected);
    ...
    const clipFileInput = CUtil.ID("clipFileInput") as HTMLSelectElement;
    ...
    const loopCheck = CUtil.ID("loopCheck") as HTMLInputElement;
    loopCheck.checked = gAni.mLoop ?? false; // 초기 상태 반영
...
async function AniToolDrop(_drop : CDrop)
{
...
function DragBoxToPoint(_bound: CBound) {
    ...
    if (stRay.GetDirect().z == 0 || edRay.GetDirect().z == 0) return;
    ...
    //opengl coord => paint coord
    // let pt=gSubject.FindComp(CPaint2D);
    // let ptSize = new CVec3(pt.GetSize().x, pt.GetSize().y);
    // let curStartPoint = CMath.V3SubV3(GAniEngin.obj.GetPos(), CMath.V3MulFloat(ptSize, 0.5));
...
function AniToolUpdate(_delay) {
    if (gAtl.Frame().Input().KeyDown(CInput.eKey.LButton)) {
        ...
        if (gMode == 4) {
            ...
            let codi = gAni.mClip[Number(gSelect.Key())] as CClipCoodi;
...
function AniToolSetMode(_mode: number) {
    ...
    switch (gMode) {
        case 1: CUtil.ID("ModeMove_btn").className = "btn btn-primary"; break;
        case 2: CUtil.ID("ModeCreate_btn").className = "btn btn-success"; break;
        case 3: CUtil.ID("ModeDelete_btn").className = "btn btn-danger"; break;
        case 4: CUtil.ID("ModeRange_btn").className = "btn btn-secondary"; break;
    }
}
...
function AniToolRangeInit() {
    ...
    let pt = ClipSub.PushComp(new CPaintHTML(CDomFactory.DataToDom(`<div class='border border-primary' style='color:red;font-size: x-small;'></div>`),
        new CVec2(EDX_num - STX_num, EDY_num - STY_num), CUtil.ID("AniToolLeft_div")));
...
async function AniToolSubjectInit() {
    ...
    for (let i = 0; i < gAni.mClip.length; ++i) {
        ...
        if (clip instanceof CClipImg) {
            ...
            let ClipSub = gAtl.Canvas("AniTool").PushSub(new CSubject());
            ClipSub.SetKey(i);
            let pt = ClipSub.PushComp(new CPaintHTML(CDomFactory.DataToDom(`<div class='border border-primary' style='color:red;font-size: x-small;'>${i}</div>`),
                new CVec2(clip.mEDX - clip.mSTX, clip.mEDY - clip.mSTY), CUtil.ID("AniToolLeft_div")));
            //pt.SetPivot(new CVec3(1,-1,1));
        ...
        {
            let mesh=gAtl.Frame().Res().Find(clip.mMesh) as CMesh;
...
function AniToolViewRender() {
    ...
    for (let i = 0; i < gAni.mClip.length; ++i) {
        ...
        block.style.whiteSpace = "normal";    // 여러 줄 허용
        block.style.wordBreak = "break-all";  // 단어 단위 강제 줄바꿈
        block.textContent = `${clip.constructor.name}[${i}]`;
        //block.title = `Clip ${i} (${clip.mTime}ms ~ ${clip.mTime + clip.mDelay}ms)`;
...
function AniToolAniListInit() {
    ...
    for (let i = 0; i < gAni.mClip.length; ++i) {
        ...
        btnDown.addEventListener("click", () => {
            if (i >= gAni.mClip.length - 1) return;
        ...
        btnClose.addEventListener("click", () => {
            gAni.mClip.splice(i, 1);           // 데이터에서 삭제
            AniToolAniListInit();              // 다시 렌더링
        });
    ...
    AniToolViewRender(); // 타임라인 렌더링
    AniToolSubjectInit();
}


//CObject.PushEditerBtn(new CObjectEditerBtn("AniTool","AniTool"));