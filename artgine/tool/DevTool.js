...
function DevToolRender() {
    ...
    let FunRenSub = (_sub, _font = false) => {
        ...
        color.w = SDF.eColorModel.RGBAdd;
        for (let cl of clArr) {
            if (cl.GetOwner() == null || cl.IsEnable() == false || cl.GetLayer() == "")
                continue;
            let bound = cl.GetBW();
        ...
        for (let lt of ltArr) {
            if (lt.GetOwner() == null || lt.GetOwner().IsEnable() == false || lt.IsPointLight() == false)
                continue;
    ...
    let shader = gAtl.Frame().Pal().Sl3D().GetShader("Artgine/Shader/3DSimpleCMAM");
    let meshDrawBox = gAtl.Frame().Res().Find(gAtl.Frame().Pal().GetDevBoxMesh() + "Dev");
    if (meshDrawBox == null) {
        let mesh = gAtl.Frame().Res().Find(gAtl.Frame().Pal().GetDevBoxMesh());
                ...
                if (pathLoad) {
                    ...
                    for (let i = 0; i < fileDrop.mFiles.length; ++i) {
                        ...
                        if (ext === "gltf" || ext === "fbx" || ext === "glb" || ext === "obj") {
                            ...
                            textureMap.set(file.name.toLowerCase(), fileDrop.mObject[i]);
                    ...
                    for (let i = 0; i < fileDrop.mFiles.length; ++i) {
                        ...
                        await gAtl.Frame().Load().LoadSwitch(fileDrop.mObject[i], base64.mData, base64.mOption);
                        const mesh = gAtl.Frame().Res().Find(fileDrop.mObject[i]);
