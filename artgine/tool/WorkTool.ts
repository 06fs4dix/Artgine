...
import { CWFAction, CWFCondition, CWFOperator, CWFSystem, CWFSystemMulti, CWFSystemSingle, CWFTask, CWorkFlow } from "../util/CWorkFlow.js";
import { SDF } from "../z_file/SDF.js";

var gModal: CModalFlex;
var gAtl: CAtelier;
var gWF : CWFSystem;
var gToolTip : CTooltipListAuto;
var gUpdateEvent: CEvent;

export function WorkTool(_wf: CWFSystem) 
{
    gWF = _wf;
...
function WFTUpdate(_delay) {
    if (gWF instanceof CWFSystemSingle) {
        let wfs = gWF as CWFSystemSingle;
        ...
        for (let task of wfs.mTaskList) {
            ...
            if (isActive) //{
                el.style.border = "3px solid red"; // 활성화된 카드 강조
...
function WFTActionInit(_task : CWFTask,_action: CWFAction,_seed=1): HTMLElement {
    ...
    // 시간(ms) -> "XmYs" 형식 문자열로 변환
    const miliSecToMiniteStr = (ms: number): string => {
        if (ms === -1) return "";
        let sec = ms / 1000;
    ...
    const updateLoopTitle = () => {
        const start = parseInt((html.querySelector(`#input_mStart_${_action.ObjHash(_seed)}`) as HTMLInputElement)?.value || "-1");
        const end = parseInt((html.querySelector(`#input_mEnd_${_action.ObjHash(_seed)}`) as HTMLInputElement)?.value || "-1");
        const delay = parseInt((html.querySelector(`#input_mDelay_${_action.ObjHash(_seed)}`) as HTMLInputElement)?.value || "-1");
        const count = parseInt((html.querySelector(`#input_mCount_${_action.ObjHash(_seed)}`) as HTMLInputElement)?.value || "-1");
        ...
        if (labelEl) labelEl.textContent = title || "Loop";
    };

    // 인풋 이벤트 바인딩 + 데이터 반영 + 타이틀 갱신
    ["mStart", "mDelay", "mCount", "mEnd"].forEach(member => {
        const input = html.querySelector(`#input_${member}_${_action.ObjHash(_seed)}`) as HTMLInputElement;
        if (input) {
            input.addEventListener("change", () => {
                const val = parseInt(input.value.trim());
                if (!isNaN(val)) (_action as any)[member] = val;
                updateLoopTitle();
            });
        }
    });
    ...
    // 기존 코드 아래에 추가: textarea 입력 → 내부 데이터 반영
    ...
    const deleteBtn = html.querySelector(`#btn_delete_action_${_action.ObjHash(_seed)}`) as HTMLButtonElement;
    deleteBtn?.addEventListener("click", (e) => {
        e.stopPropagation(); // collapse 영향 방지
...
function WFTConditionInit(_task : CWFTask,_condition: CWFCondition, _seed = 1): HTMLElement {
    ...
    const select = html.querySelector(`#logic_${_condition.ObjHash(_seed)}`) as HTMLSelectElement;
    ...
    const input = html.querySelector(`#link_${_condition.ObjHash(_seed)}`) as HTMLInputElement;
    input.addEventListener("change", () => {
        _condition.mLink = input.value.trim();
        WFTLeftInit();
    });
    // ✅ Link 자동완성 datalist 추가
    ...
    const container = html.querySelector(`#container_operator_${_condition.ObjHash(_seed)}`) as HTMLElement;
    ...
    const refreshOperatorView = () => {
        ...
        for (let op of _condition.mOperator) {
            ...
            // opTextarea.addEventListener("change", () => {
            //     const str = opTextarea.value.trim();
            //     try {
            //         op.StrToData(str);
            //         WFTLeftInit();
            //     } catch (e) {
            //         console.warn("StrToData 파싱 실패", e);
            //     }
            // });
   ...
   if (!isReadonly) {
        const btnAdd = html.querySelector(`#btn_add_operator_${_condition.ObjHash(_seed)}`) as HTMLButtonElement;
        const btnDelete = html.querySelector(`#btn_delete_condition_${_condition.ObjHash(_seed)}`) as HTMLButtonElement;
...
function getAllTaskKeys(): string[] {
    if (gWF instanceof CWFSystemSingle) {
        return (gWF as CWFSystemSingle).mTaskList.map(t => t.mKey);
    } else {
        return (gWF as CWFSystemMulti).mTaskTree.GetArray().map(n => n.mKey);
    }
}
function WFTTaskInit(_task: CWFTask, _parent: string = null,_seed): HTMLElement 
{

 

    const collapseId = `task_body_${_task.ObjHash(_seed)}`;
    const collapseClass = _seed === 2 ? "collapse show" : "collapse";  // ✅ 변경된 부분
    ...
    if(_seed==2)
    {
        ...
        html.addEventListener('dragstart', (ev) => {
            ev.dataTransfer?.setData('sourceKey', _task.Key()); // 오른쪽 카드의 key/hash
        });
        
    }
    

    const keyInput = html.querySelector(`#input_key_${_task.ObjHash(_seed)}`) as HTMLInputElement;
    keyInput?.addEventListener("change", () => {
        const newKey = keyInput.value.trim();
        if (newKey !== "" && newKey !== _task.mKey) {
            _task.mKey = newKey;
            WFTInit(); // 전체 UI 다시 초기화
        }
    });
    ...
    const parentInput = html.querySelector(`#input_parent_${_task.ObjHash(_seed)}`) as HTMLInputElement;
    if (parentInput) 
    {
    ...
    const containerAction = html.querySelector(`#container_action_${_task.ObjHash(_seed)}`) as HTMLElement;
    const containerCondition = html.querySelector(`#container_condition_${_task.ObjHash(_seed)}`) as HTMLElement;
    ...
    if (_seed != 2) 
    {
        const btnAddAction = html.querySelector(`#btn_add_action_${_task.ObjHash(_seed)}`) as HTMLButtonElement;
        ...
        const btnAddCondition = html.querySelector(`#btn_add_condition_${_task.ObjHash(_seed)}`) as HTMLButtonElement;
    ...
    else 
    {
        ...
        // collapse toggle 속성 제거 (헤더에서 동작 못하게 함)
        const collapseHeaders = html.querySelectorAll(".card-header");
        collapseHeaders.forEach((collapseHeader: Element) => {
            ...
            (collapseHeader as HTMLElement).style.cursor = "default";
            (collapseHeader as HTMLElement).style.opacity = "0.6";
        });

        // 버튼 숨기기
        const addButtonGroup = html.querySelector(`#${collapseId} .card-body > .d-flex`);
        if (addButtonGroup) {
            addButtonGroup.remove();  // 버튼 div 통째로 제거
        }

        const inputs = html.querySelectorAll("input, textarea, select, button");
        inputs.forEach(el => {
            if (el.tagName === "BUTTON") {
                (el as HTMLButtonElement).hidden = true;
        ...
        html.addEventListener("click", () => {
            ...
            if (target) {
                ...
                (target as HTMLElement).focus(); // 강조 효과 등 추가 가능
            }
        });
    }

    const deleteBtn = html.querySelector(`#btn_delete_task_${_task.ObjHash(_seed)}`) as HTMLButtonElement;
    deleteBtn?.addEventListener("click", (e) => {
        e.stopPropagation(); // collapse toggle 방지
        
        if (gWF instanceof CWFSystemSingle) {
            const wfs = gWF as CWFSystemSingle;
...
async function WFTLeftInit_Multi(wfm: CWFSystemMulti): Promise<void> {
    ...
    const renderMap = new Map<CTree<CWFTask>, RenderItem>();
    const depthHeightMap = new Map<number, number>();

    // 1단계: 노드별 subject + paint 생성 및 depth 기록
    function buildList(node: CTree<CWFTask> | null, depth = 0): void {
        if (!node) return;
    ...
    // 2단계: 사이즈 측정 및 depth별 최대 height 기록
    for (const item of renderMap.values()) 
    {
        await CChecker.Exe(async ()=>{
            if(item.pt.mAttach) return false;
            return true;
        },1);
    ...
    // 3단계: depth마다 부모+자식 높이 고려한 totalHeight 생성
    const depthTotalHeightMap = new Map<number, number>();
    for (const [depth, height] of depthHeightMap) 
    {
        let pHeight = depthHeightMap.get(depth -1) || 0;
        // if(depth>0)
        //pHeight+=(depth+1)*safeSpacing;
    ...
    // 5단계: subtreeWidth 계산
    function calcSubtreeWidth(node: CTree<CWFTask> | null): number {
        if (!node) return 0;
        ...
        if (node.mChilde) width -= safeSpacing;
    ...
    // 6단계: 최종 layout (x 누적, y 고정)
    function layout(node: CTree<CWFTask> | null, startX: number, depth: number): number {
        let x = startX;
    ...
    for (let task of wfm.mTaskTree.GetArray()) 
    {
        ...
        // if(task.mColleague!=null)
        // {
        //     let cSub=gAtl.Canvas("WorkTool").Find(task.mColleague.mKey);
        //     const line = gAtl.Canvas("WorkTool").Push(new CSubject());
        //     let trail=line.PushComp(new CPaintTrail(gAtl.Frame().Pal().GetNoneTex()));
        //     trail.Arrow();
        //     trail.SetStaticPosList([tSub.GetPos(),cSub.GetPos()]);
        //     trail.SetColorModel(new CColor(1,1,1,CColor.eModel.RGBAdd));
        // }
...
function WFTLinkLine(_task : CWFTask)
{
...
async function WFTLeftInit() {
    ...
    if (gWF instanceof CWFSystemSingle) {
        ...
        const cards: { sub: CSubject, pt: CPaintHTML, el: HTMLElement }[] = [];
        ...
        const angleStep = (2 * Math.PI) / taskCount; // 각도 2배
        let sumX = 0;
        let sumY = 0;
        ...
        for(let i=0;i<cards.length;++i)
        {
            ...
            const expandFactor = 1.2; // 💡 퍼짐 정도 조절 (1.0 = 기존 거리, 2.0 = 2배 멀리)
        ...
        // const center = new CVec3(centerX, centerY, 0);
        ...
        if(cards.length>0)
        {
            ...
            //gAtl.Brush().GetCam2D().EyeMoveAndViewCac();
...
function WFTRightInit() {
    ...
    const input = CUtil.ID("wf_add_input") as HTMLInputElement;
    const button = CUtil.ID("wf_add_btn") as HTMLButtonElement;
...
function CutToolTip(_str: string, _index: number): string 
{
    let i = _index - 1;
    let depth = 0;

    // 커서 기준 왼쪽으로 스캔하면서 메서드 체인 파싱
    while (i >= 0) {
        const ch = _str[i];
...
function ToolTipAttach(_input : HTMLInputElement|HTMLTextAreaElement)
{
    // ✂️ 커서 기준 CutToolTip 실행
    function logCutResult() {
        ...
        else 
        {
            ...
            if (typeof target === "function") 
            {
                ...
                // let enName=CClass.EnumName(target);
                // for(let key of enName)
                // {
                //     list.push([key,"Enum"]);
                // }
        ...
        gToolTip=new CTooltipListAuto(
            list,_input,CTooltip.eTrigger.Manual);
        gToolTip.Open();
        gToolTip.SetMaxWidth(400);
		gToolTip.SetMaxHeight(400);
        //gToolTip.SetSize(0,0);
        // showAutocomplete(result); // 자동완성 연동시 사용
    }

    // 📌 포커스 또는 마우스 클릭
    //textarea.addEventListener("focus", logCutResult);
    _input.addEventListener("mouseup", logCutResult);

    //📌 키보드 커서 이동 시
    _input.addEventListener("keyup", (e : any) => {
    ...
    _input.addEventListener("input", () => {
        ...
        {
            logCutResult(); // 자동완성 트리거 가능
        }
    });
    _input.addEventListener("blur", () => {
        //const str = _input.value.trim();
        //_action.StrToData(str);
        WFTLeftInit();
