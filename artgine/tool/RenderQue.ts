...
function Init() {
    ...
    for (let [key, info] of g_roBrush.mRenInfoMap) 
    {
        ...
        if (rp.mSort != null) {
            ...
            else if (rp.mSort === CRenderPass.eSort.RPAlphaGroup) sortStr = "RPAlphaGroup";
        ...
        sub.SetPos(new CVec3(posX, posY));
        ptArr.push(pt);
        //posY=pt.
        posX += 280;
    }
    ...
    .sort((a, b) => a[0] - b[0]); // priority 기준 정렬
     ...
     for (let [priority, renPri] of sortedRenPriEntries) {
        ...
        //     const rp = alphaList.Find(i);
        //     alphaHTML += `
        //         <li class="list-group-item p-1">
        //             <div>key: ${rp.mRenInfoKey ?? "-"}</div>
        //             <div>show: ${rp.mShow}</div>
        //             <div>type: ${rp.mPaint?.constructor?.name ?? "-"}</div>
        //         </li>
        //     `;
        // }
        let prev = null;
