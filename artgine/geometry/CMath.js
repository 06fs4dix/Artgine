...
export class CMath {
    ...
    static MatTranspose(pa_mat, pa_out = null) {
        if (pa_out == null) {
            pa_out = new CMat();
        }
    ...
    static MatMulFloat(pa_val1, pa_val2, _out = null) {
        if (_out == null)
            _out = new CMat();
        _out.SetUnit(false);
        for (var i = 0; i < 16; ++i) {
    ...
    static V4Interpolate(_first, _second, pa_time, pa_out = new CVec4()) {
        pa_out.x = this.FloatInterpolate(_first.x, _second.x, pa_time);
        pa_out.y = this.FloatInterpolate(_first.y, _second.y, pa_time);
        pa_out.z = this.FloatInterpolate(_first.z, _second.z, pa_time);
    ...
    static Vec3toPlaneEach(pa_vec1, pa_vec2, pa_vec3) {
        var pa_out = new CVec4();
    ...
    static PlaneEachDotV3Normal(pa_plane, pa_vec) {
        return CMath.V3Dot(new CVec3(pa_plane.x, pa_plane.y, pa_plane.z), pa_vec);
    }
    static LookAt(_eye, _look, _up, _out = new CMat()) {
