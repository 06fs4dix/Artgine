...
export class CMath
{
	...
	static Clamp(_val:number,_min:number,_max:number):number {
		return Math.min(_max, Math.max(_val, _min));
	}
	static Abs(_val:number):number
	{
		return Math.abs(_val);
	}
	
	static Vec2MinusVec2(_a:CVec2, _b:CVec2):CVec2
	{
		return new CVec2(_a.mF32A[0]- _b.mF32A[0], _a.mF32A[1] - _b.mF32A[1]);
	}
	static XYLen(_ax:number, _ay:number, _bx:number, _by:number):number
	{
		var xdir = _ax - _bx;
		var ydir = _ay - _by;
	...
	//Vec3=======================================================================
	static V3MulV3(_a : CVec3,_b : CVec3,_c : CVec3=null):CVec3
	{
		if(_c != null)
		{	
	...
	static V3AddV3(_a : CVec3,_b : CVec3,_c : CVec3=null):CVec3
	{
		if(_c != null)
		{	
	...
	static V3SubV3(_a : CVec3,_b : CVec3,_c : CVec3=null):CVec3
	{
		if( _c != null)
		{
			_c.mF32A[0]=_a.mF32A[0]-_b.mF32A[0];
	...
	static V3MulFloat(_a : CVec3, _b : number,_c : CVec3=null):CVec3
	{
		if(_c != null)
		{
	...
	static V3Dot(_a:CVec3,_b:CVec3):number
	{
		return _a.mF32A[0]*_b.mF32A[0]+_a.mF32A[1]*_b.mF32A[1]+_a.mF32A[2]*_b.mF32A[2];
	}
	
	static V3Cross(_a:CVec3,_b:CVec3,_c:CVec3=null):CVec3
	{
		if(_c != null)
		{
	...
	static V3TwoAngle(_a:CVec3,_b:CVec3):number
	{
	...
	static V3SignedAngle(_a : CVec3, _b : CVec3, _axis : CVec3):number
	{
	...
	static V4AddV4(v1:CVec4, v2:CVec4) {
		return new CVec4(v1.x + v2.x, v1.y + v2.y, v1.z + v2.z, v1.w + v2.w);
	}
	
	static V4SubV4(v1:CVec4, v2:CVec4) {
		return new CVec4(v1.x - v2.x, v1.y - v2.y, v1.z - v2.z, v1.w - v2.w);
	}
	
	static V4MulFloat(v:CVec4, s:number) {
	...
	static V3DistancePseudo(_a : CVec3, _b : CVec3)
	{
	...
	// static Distance(_a:CVec3,_b:CVec3):number
	// {
	...
	// static MatMul(pa_val1 : CMat,pa_val2: CMat,pa_val3: CMat=null,_mat43=false)
	// {
	...
	static MatMul(pa_val1: CMat, pa_val2: CMat, pa_val3: CMat = null, _mat43 = false)
	{
		...
		else
		{
			...
			if (_mat43) {
				...
				L_matrix.mF32A[ 1] = a01 * b00 + a11 * b01 + a21 * b02 + a31 * b03;
				L_matrix.mF32A[ 2] = a02 * b00 + a12 * b01 + a22 * b02 + a32 * b03;
				L_matrix.mF32A[ 3] = 0.0;  // 사용 안함

				// row 1, col 0~2
				L_matrix.mF32A[ 4] = a00 * b10 + a10 * b11 + a20 * b12 + a30 * b13;
				L_matrix.mF32A[ 5] = a01 * b10 + a11 * b11 + a21 * b12 + a31 * b13;
				L_matrix.mF32A[ 6] = a02 * b10 + a12 * b11 + a22 * b12 + a32 * b13;
				L_matrix.mF32A[ 7] = 0.0;  // 사용 안함

				// row 2, col 0~2
				L_matrix.mF32A[ 8] = a00 * b20 + a10 * b21 + a20 * b22 + a30 * b23;
				L_matrix.mF32A[ 9] = a01 * b20 + a11 * b21 + a21 * b22 + a31 * b23;
				L_matrix.mF32A[10] = a02 * b20 + a12 * b21 + a22 * b22 + a32 * b23;
				L_matrix.mF32A[11] = 0.0;  // 사용 안함

				// row 3, col 0~2 (translation 등)
				L_matrix.mF32A[12] = a00 * b30 + a10 * b31 + a20 * b32 + a30 * b33;
				L_matrix.mF32A[13] = a01 * b30 + a11 * b31 + a21 * b32 + a31 * b33;
				L_matrix.mF32A[14] = a02 * b30 + a12 * b31 + a22 * b32 + a32 * b33;
				L_matrix.mF32A[15] = 1.0;  // 사용 안함: 보통 1 고정
			}
			else {
				// 기존 4×4 일반 경로
				L_matrix.mF32A[ 0] = a00 * b00 + a10 * b01 + a20 * b02 + a30 * b03;
	...
	static MatAxisToRotation(axis : CVec3, radianAngle : number,pa_out : CMat=new CMat)
	{
		...
		var L_d= 1-L_c;
		pa_out.mF32A[0]=(L_d*(axis.x*axis.x)) + L_c;pa_out.mF32A[1]=(L_d*axis.x*axis.y) + (axis.z*L_s);
		pa_out.mF32A[2]=(L_d*axis.x*axis.z) - (axis.y*L_s);pa_out.mF32A[3]=0;
	
		pa_out.mF32A[4]=(L_d*axis.x*axis.y) - (axis.z*L_s);pa_out.mF32A[5]=(L_d*(axis.y*axis.y)) + L_c;
		pa_out.mF32A[6]=(L_d*axis.y*axis.z) + (axis.x*L_s);pa_out.mF32A[7]=0;
	
		pa_out.mF32A[8]=(L_d*axis.x*axis.y) + (axis.y*L_s);pa_out.mF32A[9]=(L_d*axis.y*axis.z)-(axis.x*L_s);
		pa_out.mF32A[10]=(L_d*(axis.z*axis.z)) + L_c;pa_out.mF32A[11]=0;
		pa_out.mF32A[12]=0;
		pa_out.mF32A[13]=0;
		pa_out.mF32A[14]=0;
		pa_out.mF32A[15]=1;
	...
	static MatScale(pa_vec : CVec3,_mat : CMat=null) : CMat
	{
		var pa_out : CMat=null;
		if(_mat==null)
			pa_out=new CMat();
		else
			pa_out=_mat;
		...
		pa_out.mF32A[0]=pa_vec.x;pa_out.mF32A[1]=0;pa_out.mF32A[2]=0;pa_out.mF32A[3]=0;
		pa_out.mF32A[4]=0;pa_out.mF32A[5]=pa_vec.y;pa_out.mF32A[6]=0;pa_out.mF32A[7]=0;
		pa_out.mF32A[8]=0;pa_out.mF32A[9]=0;pa_out.mF32A[10]=pa_vec.z;pa_out.mF32A[11]=0;
		pa_out.mF32A[12]=0;pa_out.mF32A[13]=0;pa_out.mF32A[14]=0;pa_out.mF32A[15]=1;
	...
	static MatToQut(_m : CMat) 
    {
        let pa_qut : CVec4 = new CVec4();
	...
	static V3MulMatNormal(pa_vec : CVec3,pa_mat : CMat,_out  : CVec3=null) : CVec3
	{
		var pa_out=_out;
		if(pa_out==null)
			pa_out=new CVec3();
		...
		// pa_out.x = (pa_mat.m_F32A[0] * pa_vec.x) + (pa_mat.m_F32A[4] * pa_vec.y) + (pa_mat.m_F32A[8] * pa_vec.z);
		// pa_out.y = (pa_mat.m_F32A[1] * pa_vec.x) + (pa_mat.m_F32A[5] * pa_vec.y) + (pa_mat.m_F32A[9] * pa_vec.z);
		// pa_out.z = (pa_mat.m_F32A[2] * pa_vec.x) + (pa_mat.m_F32A[6] * pa_vec.y) + (pa_mat.m_F32A[10] * pa_vec.z);
	...
	static V3MulMatCoordi(pa_vec : CVec3,pa_mat : CMat,_out : CVec3=null) : CVec3
	{
		var pa_out=_out;
		if(pa_out==null)
			pa_out=new CVec3();
		else if(pa_vec==_out)	CAlert.E("V3MulMatCoordi error");
	...
	static MatInvert(pa_val1 : CMat,pa_val2 : CMat=null)
	{
	...
	// 	for (let i = 0; i < 4; i++) {
	// 		src[i]        = pa_val1.m_F32A[i*4+0];
	// 		src[i + 4]    = pa_val1.m_F32A[i*4+1];
	// 		src[i + 8]    = pa_val1.m_F32A[i*4+2];
	// 		src[i + 12]   = pa_val1.m_F32A[i*4+3];
	...
	// 	dst.m_F32A[0]  = tmp[0]*src[5] + tmp[3]*src[6] + tmp[4]*src[7];
	// 	dst.m_F32A[0] -= tmp[1]*src[5] + tmp[2]*src[6] + tmp[5]*src[7];
	// 	dst.m_F32A[1]  = tmp[1]*src[4] + tmp[6]*src[6] + tmp[9]*src[7];
	// 	dst.m_F32A[1] -= tmp[0]*src[4] + tmp[7]*src[6] + tmp[8]*src[7];
	// 	dst.m_F32A[2]  = tmp[2]*src[4] + tmp[7]*src[5] + tmp[10]*src[7];
	// 	dst.m_F32A[2] -= tmp[3]*src[4] + tmp[6]*src[5] + tmp[11]*src[7];
	// 	dst.m_F32A[3]  = tmp[5]*src[4] + tmp[8]*src[5] + tmp[11]*src[6];
	// 	dst.m_F32A[3] -= tmp[4]*src[4] + tmp[9]*src[5] + tmp[10]*src[6];
	// 	dst.m_F32A[4]  = tmp[1]*src[1] + tmp[2]*src[2] + tmp[5]*src[3];
	// 	dst.m_F32A[4] -= tmp[0]*src[1] + tmp[3]*src[2] + tmp[4]*src[3];
	// 	dst.m_F32A[5]  = tmp[0]*src[0] + tmp[7]*src[2] + tmp[8]*src[3];
	// 	dst.m_F32A[5] -= tmp[1]*src[0] + tmp[6]*src[2] + tmp[9]*src[3];
	// 	dst.m_F32A[6]  = tmp[3]*src[0] + tmp[6]*src[1] + tmp[11]*src[3];
	// 	dst.m_F32A[6] -= tmp[2]*src[0] + tmp[7]*src[1] + tmp[10]*src[3];
	// 	dst.m_F32A[7]  = tmp[4]*src[0] + tmp[9]*src[1] + tmp[10]*src[2];
	// 	dst.m_F32A[7] -= tmp[5]*src[0] + tmp[8]*src[1] + tmp[11]*src[2];
	...
	// 	dst.m_F32A[8]  = tmp[0]*src[13] + tmp[3]*src[14] + tmp[4]*src[15];
	// 	dst.m_F32A[8] -= tmp[1]*src[13] + tmp[2]*src[14] + tmp[5]*src[15];
	// 	dst.m_F32A[9]  = tmp[1]*src[12] + tmp[6]*src[14] + tmp[9]*src[15];
	// 	dst.m_F32A[9] -= tmp[0]*src[12] + tmp[7]*src[14] + tmp[8]*src[15];
	// 	dst.m_F32A[10] = tmp[2]*src[12] + tmp[7]*src[13] + tmp[10]*src[15];
	// 	dst.m_F32A[10]-= tmp[3]*src[12] + tmp[6]*src[13] + tmp[11]*src[15];
	// 	dst.m_F32A[11] = tmp[5]*src[12] + tmp[8]*src[13] + tmp[11]*src[14];
	// 	dst.m_F32A[11]-= tmp[4]*src[12] + tmp[9]*src[13] + tmp[10]*src[14];
	// 	dst.m_F32A[12] = tmp[2]*src[10] + tmp[5]*src[11] + tmp[1]*src[9];
	// 	dst.m_F32A[12]-= tmp[4]*src[11] + tmp[0]*src[9] + tmp[3]*src[10];
	// 	dst.m_F32A[13] = tmp[8]*src[11] + tmp[0]*src[8] + tmp[7]*src[10];
	// 	dst.m_F32A[13]-= tmp[6]*src[10] + tmp[9]*src[11] + tmp[1]*src[8];
	// 	dst.m_F32A[14] = tmp[6]*src[9] + tmp[11]*src[11] + tmp[3]*src[8];
	// 	dst.m_F32A[14]-= tmp[10]*src[11] + tmp[2]*src[8] + tmp[7]*src[9];
	// 	dst.m_F32A[15] = tmp[10]*src[10] + tmp[4]*src[8] + tmp[9]*src[9];
	// 	dst.m_F32A[15]-= tmp[8]*src[9] + tmp[11]*src[10] + tmp[5]*src[8];
		
	// 	det=src[0]*dst.m_F32A[0]+src[1]*dst.m_F32A[1]+src[2]*dst.m_F32A[2]+src[3]*dst.m_F32A[3];
		
	// 	det = 1/det;
	// 	for (let j = 0; j < 4; j++)
	// 	{
	// 		dst.m_F32A[j*4+0]*=det;
	// 		dst.m_F32A[j*4+1]*=det;
	// 		dst.m_F32A[j*4+2]*=det;
	// 		dst.m_F32A[j*4+3]*=det;
	// 	}
	...
	static MatTranslation(pa_vec : CVec3, _mat : CMat=null) : CMat
	{
		...
		pa_out.mF32A[4] = 0; pa_out.mF32A[5] = 1; pa_out.mF32A[6] = 0; pa_out.mF32A[7] = 0;
		pa_out.mF32A[8] = 0; pa_out.mF32A[9] = 0; pa_out.mF32A[10] = 1; pa_out.mF32A[11] = 0;
		pa_out.mF32A[12] = pa_vec.x; pa_out.mF32A[13] = pa_vec.y; pa_out.mF32A[14] = pa_vec.z; pa_out.mF32A[15] = 1;
	...
	static MatDecomposeRotMat(pa_viewMat : CMat,pa_x:boolean,pa_y:boolean,pa_z:boolean)
	{
		...
		if (pa_x)
		{
			pa_outMat.mF32A[5] = pa_viewMat.mF32A[5]; pa_outMat.mF32A[6] = pa_viewMat.mF32A[6];
			pa_outMat.mF32A[9] = pa_viewMat.mF32A[9]; pa_outMat.mF32A[10] = pa_viewMat.mF32A[10];
		}
		if (pa_y)
		{
			pa_outMat.mF32A[0] = pa_viewMat.mF32A[0]; pa_outMat.mF32A[2] = pa_viewMat.mF32A[2];
			pa_outMat.mF32A[8] = pa_viewMat.mF32A[8]; pa_outMat.mF32A[10] = pa_viewMat.mF32A[10];
		}
		if (pa_z)
		{
			pa_outMat.mF32A[0] = pa_viewMat.mF32A[0]; pa_outMat.mF32A[1] = pa_viewMat.mF32A[1];
	...
	static MatDecomposeSca(pa_mat : CMat,_sca=new CVec3()) : CVec3 {
		...
		// for(var i = 0; i < 3; i++) {
		// 	for(var j = 0; j < 3; j++) {
		// 		rotation.m_F32A[i * 4 + j] = pa_mat.m_F32A[i * 4 + j] / (j == 0? scale.x : (j == 1? scale.y : scale.z));
		// 	}
		// }
	...
	static RadianToDegree(pa_val:number)
	{
		return parseInt(((180 * pa_val) / d_PI)+"");
	}
	static EulerToQut(pa_radian : CVec3,_qut : CVec4=null,_order="YZX")
	{
		...
		//https://github.com/mrdoob/three.js/blob/master/src/math/Quaternion.js#L201
		...
		// var qyaw=new CVec4();
		// var qroll=new CVec4();
		// qroll = CMath.QutAxisToRotation(new CVec3(1, 0, 0), pa_radian.x);
		// qpitch = CMath.QutAxisToRotation(new CVec3(0, 1, 0), pa_radian.y);
		// qyaw = CMath.QutAxisToRotation(new CVec3(0, 0, 1), pa_radian.z);
		// qroll = CMath.QutMul(qroll, qyaw);
		// CMath.QutMul(qroll, qpitch,pa_qut);
	
		return pa_qut;
	}

	static QutToEuler(pa_qut:CVec4,_order="YZX")
	{
		...
		// pa_radian.x = Math.atan2(2.0 * (w * x + y * z), 1.0 - 2.0 * (sqx + sqy));
		// pa_radian.y = 2.0 * Math.atan2(Math.sqrt(1.0 + 2.0 * (w * y - x * z)), Math.sqrt(1.0 - 2.0 * (w * y - x * z))) - Math.PI / 2;
		// pa_radian.z = Math.atan2(2.0 * (w * z + x * y), 1.0 - 2.0 * (sqy + sqz));
	
		//original
		// pa_radian.x = Math.atan2(2.0 * (pa_qut.w*pa_qut.x - pa_qut.y*pa_qut.z), 1.0 - 2.0 * (sqx + sqy)); // rotation about x-axis
	...
	static QutMul(pa_val1 : CVec4, pa_val2 : CVec4,pa_val3 : CVec4=null)
	{	
		var L_qut=new CVec3();
		var L_Oqut=pa_val3;
		if(L_Oqut==null)
		...
		//var L_vec = CMath.V3Cross(new CVec3(pa_val1.x, pa_val1.y, pa_val1.z), new CVec3(pa_val2.x, pa_val2.y, pa_val2.z));
	...
	static QutInverse(pa_val1:CVec4)
	{
		var L_con=new CVec4();
	...
	static QutNomalize(pa_val1:CVec4)
	{
		var L_con=new CVec4();
		var L_len = 0;
		L_len = CMath.QutLenght(pa_val1);
	...
	static QutConjugate(pa_val1:CVec4)
	{
	...
	static FromToRotation(_from : CVec3, _to : CVec3) {
...
//	}
	...
	static QutToMat(pa_val1 :CVec4,_mat : CMat=null) : CMat
	{
		var pa_out : CMat=null;
		if(_mat==null)
			pa_out=new CMat();
...
//	static QutToMatrix(pa_val1,_mat=null)
//	{
//		var pa_out=null;
//		if(_mat==null)
//			pa_out=new CMat();
...
//		return pa_out;
//	}
	static QutDot(pa_val1:CVec4, pa_val2:CVec4)
	{
		return pa_val1.x*pa_val2.x + pa_val1.y*pa_val2.y + pa_val1.z*pa_val2.z + pa_val1.w*pa_val2.w;
	}
	static Vec4PlusVec4(_val1:CVec4, _val2:CVec4)
	{
		return new CVec4(_val1.x + _val2.x, _val1.y + _val2.y, _val1.z + _val2.z, _val1.w + _val2.w);
	}
	static FloatInterpolate(_first : number,_second : number,pa_time : number)
	{	
		return _first * (1.0 - pa_time) + _second * pa_time;
	}
	static V3Interpolate(_first : CVec3, _second : CVec3, pa_time : number, pa_out : CVec3 = new CVec3())
	{
	...
	static QutInterpolate(pa_first:CVec4,pa_second:CVec4, pa_time:number,pa_out=new CVec4())
	{
		...
		//pa_first=pa_first.Export();
		var		omega, cosom, sinom, scale0, scale1;
		cosom=CMath.QutDot(pa_first, pa_second); // = cos(θ)
