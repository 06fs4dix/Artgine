...
export class CObject implements IMember,IRecycle,IStream,ICJSON
{
	...
	private static lKeyCache = new Map<string, string>();

	private static ToTempKey(_key: string): string {
		if (_key[0] !== "/") return _key;
		let cached = CObject.lKeyCache.get(_key);
		if (cached != null) return cached;
		let result = "/" + _key.slice(1).split("/").map(s =>
			s.length > 1 && s[0] !== s[0].toLowerCase() ? s[1].toLowerCase() + s.slice(2) : s
		).join("/");
		CObject.lKeyCache.set(_key, result);
		return result;
	}

	Temp(_key: string, _value = null) {
		_key = CObject.ToTempKey(_key);
		if (this["mTemp"] == null) this["mTemp"] = {};
		if (_value != null) this["mTemp"][_key] = _value;
