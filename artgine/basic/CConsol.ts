...
var gLogEvent : Function=null;
var gConsolLog=new CQueue<string>();
export class CConsol
{
	...
	static SetLogEvent(_event : Function) 
	{
		gLogEvent=_event;
	}
	static GetLogQue()
	...
	static Log(_text,_color=null)
	{
		...
		if(CUtil.IsNode())
		{
			if(_color==null)
			{
				...
				console.log("\x1b[37m" + _text+"\x1b[0m"); // ✅ 완전 초기화
			}
			else
			{
				const colorCode = CConsol.NodeColor(_color);
				
				console.log(colorCode + _text + "\x1b[0m"); // \x1b[0m: 리셋
			}
	...
	static NodeColor(_color: string = ''): string {
		switch (_color?.toLowerCase()) {
			...
			default: return ""; // 기본 콘솔 색상
		}
	}
	static WebColor(_color: string = ''): string {
		if (!(_color && typeof _color === 'string')) return "";
		if (_color.startsWith('#')) return _color;
		if (/^[0-9a-fA-F]{6,8}$/.test(_color)) {
			if (_color.length === 8) _color = _color.substring(2); // ARGB → RGB
			return `#${_color}`;
		}
		return _color; // red, blue 등
	}
}