...
export class CStream {
    ...
    static DefinePacket(sample) {
        let name = "";
        const keys = Object.keys(sample);
        function fn(_data, _stream = new CStream()) {
            if (_data instanceof CStream) {
                return _data.GetPacketParser(keys);
            }
            _stream.Push(name);
            for (const k of keys)
                _stream.Push(_data[k]);
            return _stream;
        }
        fn.__setName = (n) => {
            name = n;
    ...
    static RegisterPacketNames(cls) {
