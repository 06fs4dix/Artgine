...
export class CObject {
    ...
    ProxyTARPush(_target, _pointer = null) {
    ...
    IsShould(_member, _type) {
        if (this.IsProxy() && _type == CObject.eShould.Editer) {
            if (this.IsShould(_member, CObject.eShould.Proxy) == true)
                return false;
        }
        if (_type == CObject.eShould.Watch)
            return false;
        if (_member.indexOf('mObject') != -1 || _member == 'class' || _member == "mProxy" ||
    ...
    Watch(_event) {
        ...
        for (const member in this) {
            if (!this.IsShould(member, CObject.eShould.Watch))
                continue;
            ...
            if (Array.isArray(val))
                val = this.WatchArray(val, member, watch);
    ...
    static lKeyCache = new Map();
    static ToTempKey(_key) {
        if (_key[0] !== "/")
            return _key;
        let cached = CObject.lKeyCache.get(_key);
        if (cached != null)
            return cached;
        let result = "/" + _key.slice(1).split("/").map(s => s.length > 1 && s[0] !== s[0].toLowerCase() ? s[1].toLowerCase() + s.slice(2) : s).join("/");
        CObject.lKeyCache.set(_key, result);
        return result;
    }
    Temp(_key, _value = null) {
        _key = CObject.ToTempKey(_key);
        if (this["mTemp"] == null)
            this["mTemp"] = {};
        if (_value != null)
            this["mTemp"][_key] = _value;
        return this["mTemp"][_key];
    }
