...
let gPitchShifterCtor = null;
let gPitchShifterLoading = null;
async function LoadPitchShifterCtor() {
    ...
    if (!gPitchShifterLoading) {
        gPitchShifterLoading = import('../../external/esnext/SoundTouchJS/src/PitchShifter.js')
            .then((m) => m?.default ?? m?.PitchShifter ?? m)
            .then((ctor) => (gPitchShifterCtor = ctor));
    }
