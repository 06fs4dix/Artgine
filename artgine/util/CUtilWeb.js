...
export class CUtilWeb {
    ...
    static sNodeBaseTypeFiles = ["globals.d.ts", "globals.typedarray.d.ts", "buffer.buffer.d.ts", "process.d.ts"];
    static mNodeTypesLoaded = new Set();
    static async LoadNodeTypeFile(_relPath) {
        if (CUtilWeb.mNodeTypesLoaded.has(_relPath))
            return;
    ...
    static async LoadNodeBuiltinTypes(_module) {
        const name = _module.replace(/^node:/, "");
        const relPath = CUtilWeb.sNodeBuiltinTypeFiles[name];
        if (relPath == null)
            return;
        if (CUtilWeb.mNodeTypesLoaded.size == 0) {
            for (const f of CUtilWeb.sNodeBaseTypeFiles)
                await CUtilWeb.LoadNodeTypeFile(f);
        }
