...
import { CParserIMG } from "./CParserIMG.js";
...
export default class CParserSpine extends CParser {
    ...
    async Load(pa_fileName: string) {
        ...
        // bones
        let boneList = new Array<string>();
        let bones = this.mJSON.Get("bones");
        for (let bone of bones.GetDocument()) {
            let node = this.mMesh.meshTree.Find(bone.parent);
            if (node == null) node = this.mMesh.meshTree;

            node = node.PushChild(bone.name);
            node.mData = new CMeshDataNode();

            if (bone.x != null) node.mData.pos.x = bone.x;
            if (bone.y != null) node.mData.pos.y = bone.y;

            if (bone.scaleX != null) node.mData.sca.x = bone.scaleX;
            if (bone.scaleY != null) node.mData.sca.y = bone.scaleY;
        ...
        let slotAttachMap = new Map<string, string>();
        let slotColorMap = new Map<string, CVec4>();
        ...
        for (let attKey in attachments) {
            ...
            // 텍스쳐 여러 개일 때 바꿔줄 수 있는 로직이 필요, 현재는 하나만 적용됨
            let first = true;
            for (let texKey in att) {
                ...
                // region, 일반 사각형 모양 텍스쳐
                if (texData.type == null) {
                    let pos = new CVec3();
                    if (texData.x != null) pos.x = texData.x;
                    if (texData.y != null) pos.y = texData.y;
                    pos.z = node.mData.pos.z;

                    let sca = new CVec3(1, 1, 1);
                    if (texData.scaleX != null) sca.x = texData.scaleX;
                    if (texData.scaleY != null) sca.y = texData.scaleY;

                    // let rot=new CVec3(0,0,0);
                    // if(texData.rotation!=null) rot.z=CMath.DegreeToRadian(texData.rotation);
                ...
                // 사각형 모양이 아닌 메시
                    ...
                    if (texData.uvs.length == texData.vertices.length) {
                        ...
                        for (let i = 0; i < texData.triangles.length; ++i) {
                            ...
                            uvb.bufF.Push(new CVec2(texData.uvs[off * 2 + 0], 1 - texData.uvs[off * 2 + 1]));  // y축 반전
                            web.bufF.Push(new CVec4(1, 0, 0, 0));
                            wib.bufF.Push(new CVec4(meshskin.offset, 0, 0, 0));
                        }
                    }
                    // vertex가 vec2로 되어있지 않음
                    // [weightNum, boneIndex, posX, posY, weight]
                    else {
                        let vOffList = new Array<number>();
                        for (let j = 0; j < texData.vertices.length; j++) {
                            vOffList.push(j);
                            j += texData.vertices[j] * 4;
                        }

                        //최적화 방법이 있을거 같은데 현재는 버텍스 하나당 매트릭스 연결
                        for (let i = 0; i < texData.triangles.length; ++i) {
                            let off = texData.triangles[i];
                            let voff = vOffList[off];

                            //posb.bufF.Push(new CVec3());
                            uvb.bufF.Push(new CVec2(texData.uvs[off * 2 + 0], 1 - texData.uvs[off * 2 + 1]));  // y축 반전
            ...
            for (let slotKey in animations[aniKey].slots) {
                ...
                if (slotAni.attachment != null) {
                    for (var aData of slotAni.attachment) {
                        ...
                        if (aData.name != null) kf.value.x = this.mMesh.texture.indexOf(this.mPath + aData.name + ".png");
            ...
            for (let boneKey in animations[aniKey].bones) {
                ...
                if (boneAni.translate != null) {
                    for (var aData of boneAni.translate) {
                        ...
                        if (aData.time != null) kf.key += Math.trunc(aData.time * 3000);
                        if (aData.x != null) kf.value.x += aData.x;
                        if (aData.y != null) kf.value.y += aData.y;
                        if (aData.z != null) kf.value.z += aData.z;
        ...
        for(const [aniKey, aniInfo] of this.mMesh.aniMap) {
            // 시작 시간에 키프레임 없으면 추가
            const AddStartKF=(_kfList : CKeyFrame[], _basic : CVec3|CVec4) => 
            {
                let hasKFInMiddle = false;
                let off=_kfList.length;
                for(let i=0;i<_kfList.length;++i)
                {
                    if(_kfList[i].key == aniInfo.start) return;
            ...
            // 끝 시간에 키프레임 없으면 추가
            const AddEndKF=(_kfList : CKeyFrame[])=>
            {
                if(_kfList.length!=0)
                {
                    let off=0;
                    ...
                    {
                        if(_kfList[i].key == aniInfo.end) return;
                        if(_kfList[i].key <  aniInfo.end) {
                            off=i;
                            break;
                        }
                        if(_kfList[i].key <= aniInfo.start) {
                            break;  // 여기 들어오면 버그임, 위에서 생성해주기 때문임
                        }
                    }
            ...
            // 애니메이션 사이에 base로 돌아가는 키프레임 추가
            const AddBaseKF=(_kfList : CKeyFrame[],_basic : CVec3|CVec4)=>
            {
                let off=_kfList.length;
                for(let i=0;i<_kfList.length;++i)
                {
        ...
        if (atlBuf != null) {
            ...
            //const { pixels: src, width: srcW, height: srcH } = getSrcPixelsRGBA8(tex);

            //const out: string[] = [];
            for (const r of regions) {
                // Spine rotate:true → 아틀라스 상 사각형의 폭/높이가 뒤바뀜
...
//==========================================
// AtlasToBase64List.ts
// Spine .atlas 문자열 + 아틀라스 RGBA 버퍼(CTexture) → 각 리전을 PNG base64(data URL)로 반환.
// - DOM createElement 미사용
// - OffscreenCanvas 사용 (지원 환경 권장)
// - rotate:true (Spine 아틀라스의 90° CW 저장) 복원 포함
...
type Region = {
    ...
    rotate: boolean;
    x: number; y: number;
    w: number; h: number;       // trimmed size
    origW: number; origH: number;
    offX: number; offY: number;
};

function parseAtlas(atlas: string) {
    const lines = atlas.split(/\r?\n/).map(s => s.trimEnd());
    const regs: Array<{
        name: string; rotate: boolean;
        x: number; y: number; w: number; h: number;
        origW: number; origH: number; offX: number; offY: number;
        index?: number;
    }> = [];

    let i = 0;
    const isEmpty = (s: string) => !s || s.trim() === "";
    const nums = (v: string) => v.split(",").map(s => parseInt(s.trim(), 10));
    ...
    while (i < lines.length) {
        ...
        // 콜론 없는 라인은 "페이지명" 또는 "리전명"
        if (!line.includes(":")) {
            // 다음 비어있지 않은 라인이 페이지 속성인지 확인하여 구분
            ...
            if (isPageHeader) {
                // 페이지 속성 소비
                i = j + 1;
                while (i < lines.length && !isEmpty(lines[i]) && lines[i].includes(":")) i++;
                continue;
            }
            ...
            while (i < lines.length) {
                const s = lines[i];
                if (isEmpty(s)) { i++; break; }
                if (!s.includes(":")) break; // 다음 리전/페이지 시작
                const pair = kv(s); if (!pair) break;
                const [k, v] = pair;

                if (k === "rotate") rotate = v.toLowerCase() === "true";
                else if (k === "xy") [x, y] = nums(v);
                else if (k === "size") [w, h] = nums(v);
                else if (k === "orig" || k === "original size") [origW, origH] = nums(v);
                else if (k === "offset") [offX, offY] = nums(v);
                else if (k === "index") index = parseInt(v, 10);
                // splits/pads 등은 필요 시 추가
                i++;
            }
...
/** 회전 없음: src(sx,sy,w,h) → dst(0,0,w,h) 복사 */
function blitRGBA(
...
/** 아틀라스에 90° CW로 저장된 사각형을 정상 방향으로 되돌려 복사
 *  입력 사각형 크기: wRot(=h), hRot(=w)
 *  매핑: dst(x,y) -> src(sx + y, sy + (hRot - 1 - x))
 */
// function blitRotateCWtoUpright(
//     dst: Uint8Array, dstW: number, dstH: number,
//     src: Uint8Array, srcW: number,
//     sx: number, sy: number, wRot: number, hRot: number
// ) {
//     const BPP = 4;
//     for (let y = 0; y < dstH; y++) {
//         for (let x = 0; x < dstW; x++) {
//             const sxPix = sx + y;
//             const syPix = sy + (hRot - 1 - x);
...
function blitRotateCWtoUpright(
  ...
  for (let y=0;y<dstH;y++){
    for (let x=0;x<dstW;x++){
      ...
      if (sxPix<sx) sxPix=sx; else if (sxPix>maxX) sxPix=maxX;
      if (syPix<sy) syPix=sy; else if (syPix>maxY) syPix=maxY;
...
/** RGBA 버퍼 → PNG base64(data URL) (OffscreenCanvas 사용, ArrayBuffer 보장) */
async function rgbaToPngDataURL(rgba: Uint8Array, w: number, h: number): Promise<string> {
    // 1) 새 ArrayBuffer로 복제해 SharedArrayBuffer 가능성 제거
    //    (Uint8ClampedArray(rgba) 생성자는 복사본을 만들어 일반 ArrayBuffer를 보장)
    const clamped = new Uint8ClampedArray(rgba); // ✅ safe: copies into fresh ArrayBuffer

    // 2) OffscreenCanvas로 래스터 붙이기
    const canvas = new OffscreenCanvas(w, h);
    const ctx = canvas.getContext("2d", { willReadFrequently: true })!;
    ctx.imageSmoothingEnabled = false;
	ctx.globalCompositeOperation = "copy"; // 블렌딩/프리멀티 영향 없이 픽셀 복사
    ctx.fillStyle = '#FF0000'; 
    // TS 오버로드: (data: Uint8ClampedArray, sw: number, sh?: number)
    const imgData = new ImageData(clamped, w, h); // ✅ 이제 타입 에러 없음
...
// ArrayBuffer(PNG) -> { pixels: Uint8Array, width: number, height: number }
async function decodePNGToRGBA(ab: ArrayBuffer): Promise<{pixels: Uint8Array; width: number; height: number}> {
  // Blob -> ImageBitmap (캔버스 경유 X, 브라우저 디코더 사용)
  const blob = new Blob([ab], { type: "image/png" });
  //const img = await createImageBitmap(blob);
  const img = await createImageBitmap(blob, {
  premultiplyAlpha: 'none',          // 스트레이트로
  colorSpaceConversion: 'none'       // 원본 그대로
  // imageOrientation: 'none'         // 필요 시
});

  // OffscreenCanvas에 "copy"로 픽셀 붙이기 (색/알파 왜곡 방지)
  ...
  // Uint8ClampedArray -> Uint8Array (독립 버퍼)
  return { pixels: new Uint8Array(data.buffer.slice(0)), width, height };
}
function alphaBleedRGB(dst: Uint8Array, w: number, h: number, iters = 2, aThresh = 1) {
  const BPP = 4;
  ...
  for (let t = 0; t < iters; t++) {
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        const i = (y*w + x)*BPP;
        if (tmp[i+3] > aThresh) continue; // 거의/완전 투명만
        ...
        for (let dy=-1; dy<=1; dy++) for (let dx=-1; dx<=1; dx++) {
          ...
          if (na>bestA){bestA=na; r=tmp[j]; g=tmp[j+1]; b=tmp[j+2];}
        }
        if (bestA>=0){ dst[i]=r; dst[i+1]=g; dst[i+2]=b; /* 알파는 그대로 */ }
      }
    }
  }
}
function unpremultiplyInPlace(rgba: Uint8Array) {
  // a>0 픽셀의 RGB를 straight로 환원
  for (let i = 0; i < rgba.length; i += 4) {
    const a = rgba[i + 3];
    if (a > 0) {
      // (r*255)/a 를 반올림해 복원, 오버플로 방지
