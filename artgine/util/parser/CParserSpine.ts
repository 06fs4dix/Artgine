...
export default class CParserSpine extends CParser
{
    ...
    async Load(pa_fileName)
    {
        ...
        for(let bone of bones.GetDocument())
        {
            ...
            if(node==null)  node=this.m_mesh.meshTree;
        ...
        for(let aniKey in animations)
        {
            ...
            for(let boneKey in animations[aniKey].bones)
            {
                ...
                // stTime - 처음 keyFrame까지 생성
                let ResetChk=(_kfList : CKeyFrame[],_basic : CVec3|CVec4) => 
                {
                    let off=_kfList.length;
                    for(let i=0;i<_kfList.length;++i)
                    {
                        if(_kfList[i].key == stTime) return;
                ...
                // 마지막 keyFrame - endTime까지 생성
                let MatTimeChk=(_kfList : CKeyFrame[],_basic : CVec3|CVec4)=>
                {
                    if(_kfList.length!=0)
                    {
        ...
        // if(this.m_json.Get("ik")) {
        //     let iks = this.m_json.Get("ik").Document();
        ...
        // if(this.m_json.Get("transform")) {
        //     let transforms = this.m_json.Get("transform").GetDocument();
