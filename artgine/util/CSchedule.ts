...
export class CSchedule extends CObject
{
	...
	Execute(_tempTarget: any = null, _run = "", _update: CUpdate = null): boolean
	{
		if (_tempTarget == null) _tempTarget = this;
		return CSchedule.Update(_tempTarget, _run,this.mCount, this.mDelay, this.mStart, this.mEnd, _update);
	}

	IsEndReset(_tempTarget: any = null, _run = "")
	{
		if (_tempTarget == null) _tempTarget = this;
	...
	// 실시간 호출해줘야 갱신된다
	static Update(_tempTarget: any,_tempKey = "", count = 0, delay = 0, start = 0, end = 0, _update: CUpdate = null): boolean
	{
		if (_tempTarget["mTemp"] == null) _tempTarget["mTemp"] = {};
		...
		{
			(_tempTarget["mTemp"]["mTimer" + _tempKey] as CTimer).Delay();
			_tempTarget["mTemp"]["mCount" + _tempKey] = 0;
			_tempTarget["mTemp"]["mTime" + _tempKey] = 0;
			_tempTarget["mTemp"]["mDelay" + _tempKey] = 0;
		...
		if (delay != 0 && _tempTarget["mTemp"]["mDelay" + _tempKey] < delay) return false;
		if (_tempTarget["mTemp"]["mTime" + _tempKey] < start) return false;
	...
	static IsEndReset(_tempTarget: any, _run = "")
	{
		if (_tempTarget["mTemp"] == null) return false;
