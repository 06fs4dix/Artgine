...
export class CPalette {
    ...
    async Load(_fw) {
        if (_fw.Ren() == null)
            return;
        await _fw.Load().LoadSwitch(CPath.PHPC() + "artgine/z_file/none.png", CUtil.Base64ToArray(gNoneImg), new CLoaderOption());
        await _fw.Load().Load(CPath.PHPC() + "artgine/z_file/3D.ts");
        await _fw.Load().Load(CPath.PHPC() + "artgine/z_file/Voxel.ts");
        await _fw.Load().Load(CPath.PHPC() + "artgine/z_file/2D.ts");
        await _fw.Load().Load(CPath.PHPC() + "artgine/z_file/Post.ts");
        await _fw.Load().Load(CPath.PHPC() + "artgine/z_file/Cube.ts");
        await _fw.Load().Load(CPath.PHPC() + "artgine/z_file/Terrain.ts");
    ...
    Sl2DKey() { return CPath.PHPC() + "artgine/z_file/2D.ts"; }
    Sl3DKey() { return CPath.PHPC() + "artgine/z_file/3D.ts"; }
    SlPostKey() { return CPath.PHPC() + "artgine/z_file/Post.ts"; }
    SlCubeKey() { return CPath.PHPC() + "artgine/z_file/Cube.ts"; }
    SlVoxelKey() { return CPath.PHPC() + "artgine/z_file/Voxel.ts"; }
    SlTerrainKey() { return CPath.PHPC() + "artgine/z_file/Terrain.ts"; }
    MCI2D() {
        return this.mMCI2D;
    }
    GetNoneTex() {
        return CPath.PHPC() + "artgine/z_file/none.png";
    }
    GetBlackTex() {
        return "Black.tex";
    }
