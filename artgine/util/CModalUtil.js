...
export class CModalMusic extends CModal {
    ...
    mListEl;
    mRandomChk;
    constructor(names, paths, saveFn) {
        super(null);
        this.mNames = [...names];
        this.mPaths = [...paths];
        this.mSaveFn = saveFn;
        ...
        if (saveFn)
            this.mBody.querySelector('.mm-save-list')?.addEventListener('click', () => saveFn(this.mNames, this.mPaths));
    ...
    _DeleteTrack(index) {
        ...
        this.mAudio.pause();
        if (index < this.mLastPlay)
            this.mLastPlay--;
        else if (index === this.mLastPlay) {
            this.mLastPlay = 0;
            this.mNowPlayingEl.textContent = '';
        }
        this._Persist();
    ...
    _RefreshList() {
        ...
        this.mListEl.querySelectorAll('li').forEach((li, i) => {
            li.addEventListener('click', (e) => {
                ...
                if (del)
                    this._DeleteTrack(parseInt(del.dataset.del));
                else
                    this.Play(i);
            });
        });
