...
function registerDownloadTab(): void {
    ...
    const tabBtn = document.getElementById('download-tab');
    if (!tabBtn) return;
    // 이 플러그인은 Control.js보다 먼저 실행되므로 ctrlRequireAuthed를 등록 시점에 참조할 수 없다.
    // 이벤트 발생 시점(= Control.js 실행 이후)에 전역에서 지연 조회해 다른 탭과 동일한 인증 가드를 건다.
    tabBtn.addEventListener('show.bs.tab', (e: Event) => {
        const requireAuthed = (window as any).ctrlRequireAuthed as (() => boolean) | undefined;
        if (requireAuthed && !requireAuthed()) e.preventDefault();
    });
    tabBtn.addEventListener('shown.bs.tab', doMount);
}
