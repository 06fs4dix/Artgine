...
// 5. Point Light
{
    ...
    // Lit target (env map capture point)
    Main.PushSub(new CShape3D({
        shape: 'sphere',
        pos: new CVec3(2, 0, 0),
        sca: 3,
...
// 6. PBR Materials (3x3 배치: Y축=Roughness 0~1, Z축=Metallic 0~1)
{
    const PX = 10;
    const STEP = 6;
    function pbr(_y: number, _z: number, _r: number, _m: number, _col: CColor) {
        return new CShape3D({
            shape: 'sphere',
...
// 10. Shadow test (sphere + floor plane)
{
    Main.PushSub(new CShape3D({
        shape: 'sphere',
