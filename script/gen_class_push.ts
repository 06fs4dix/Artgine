...
const entries: { importPath: string; className: string }[] = [];
const excludeClasses = ['CClass', 'CFrame', 'CBrush']; // ❗ 제외할 클래스 목록
const excludeFiles = ['CTooltip.ts', 'CModalUtil.ts']; // ✅ 확장자 정확히
...
for (const subDir of targetDirs) {
    ...
    for (const filePath of tsFiles) 
    {
        ...
        const fileName = path.basename(filePath); // 파일명 추출
        ...
        // 한 줄씩 주석 제외 후 export class 찾기
        const lines = content.split(/\r?\n/).filter(line => !line.trim().startsWith('//'));
        const validContent = lines.join('\n');

        //const matches = [...validContent.matchAll(/export\s+class\s+(\w+)/g)];
        const matches = [
        ...
        for (const match of matches) {
            ...
            if (excludeClasses.includes(className) || entries.some(e => e.className === className)) continue;
